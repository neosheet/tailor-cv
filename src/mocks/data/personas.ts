import type { SourcePersona } from "../types"

/**
 * Two Personas built from the same Inventory.
 *
 ...
 * they select, which bullets within those jobs, which headline and summary,
 * and the order of the sections. Rendered side by side they should read as
 * two genuinely different résumés, with not one word of text duplicated
 * between them in the database.
 *
 * Line selection is by tag rather than by id — see `SourcePersonaLines`.
 */
export const personas: SourcePersona[] = [
  {
    id: "persona-backend",
    name: "Senior Backend — Nusantara",
    note: "Baseline persona. Start here and trim for the specific role.",
