 ...
 *
 * Reusable across the standalone `/personas/:id` page and embedded popups
 * (see docs/specs/16-inline-persona-editing-in-cv-tab.md). `dialogParamPrefix`
 * namespaces this panel's own dialog state when nested inside a page that
 * already manages its own unprefixed `dialog` URL param.
 */
export function PersonaEditorPanel({
  ...
  onDuplicated?: (persona: DbPersona) => void
}) {
  ...
  const persona = findPersona(personaStore, personaId)

  if (!persona) {
    return <p className="text-sm text-muted-foreground">Persona not found.</p>
  }
